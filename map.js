...
import mapboxAccessToken from './mapbox-token.js';
...
// Cached rollups for the "any time" (-1) slider position so we don't
// re-flatten ~260k trips on every input event.
let allDepartures;
let allArrivals;
...
map.on('load', async () => {
  ...
  // Precompute rollups for the unfiltered ("any time") view so toggling
  // back to -1 doesn't re-walk all ~260k trips.
